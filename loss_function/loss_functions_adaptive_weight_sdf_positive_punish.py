...
def sdf(model_output, gt, sdf_weight=3e3, inter_weight=1e2, normal_weight=1e2, grad_weight=5e1):
      ...
      inter_constraint = torch.where(gt_sdf == -1, torch.abs(pred_sdf), torch.zeros_like(pred_sdf))
      ...
      sdf_loss = ((sdf_constraint ** 2) * weights * 100).mean() * sdf_weight
      sdf_loss += (pos_violation ** 2).mean() * sdf_weight * 50  # 可先试 1.0 系数
